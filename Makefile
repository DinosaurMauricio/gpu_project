...
$(TARGETDIR)/gpu_transpose: ${MAIN}
	mkdir -p $(@D)
	$(NVCC) $< -lcublas -rdc=true -DDATA_TYPE=$(DATA_TYPE) -DFORMAT_SPECIFIER='"$(FORMAT_SPECIFIER)"' -DTILE_DIM=$(TILE_DIM) -DPRINT_TRANSPOSED_MATRIX=$(PRINT_TRANSPOSED_MATRIX) $(DATA_TYPE_FLAG)  --use_fast_math -o $@ $(INCLUDE) $(LIBS)
